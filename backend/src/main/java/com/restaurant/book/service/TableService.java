...
public class TableService {
  ...
  public List<Table> getTables(
    ...
    if (totalSeats != null) {
      tables = tables.stream().filter(t -> t.getTotalSeats() == totalSeats).toList();
    }

    // filters tables that have at least minSeats seats
    if (minSeats != null) {
      tables = tables.stream().filter(t -> t.getTotalSeats() >= minSeats).toList();
    }

    // filters tables that have no more seats than maxSeats
    if (maxSeats != null) {
      tables = tables.stream().filter(t -> t.getTotalSeats() <= maxSeats).toList();
    }

    // filters tables that match at least one preference
    if (preferences != null) {
      List<Preference> preferred =
          preferences.stream().filter(p -> p != null && p != Preference.NONE).distinct().toList();
